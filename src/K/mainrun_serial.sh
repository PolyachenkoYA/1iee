...
if [ $gpu_id -eq -1 ]
then
	$gmx_serial mdrun -deffnm $name -ntomp $ompN -dlb no -pin on -cpi $name.cpt -cpo $name.cpt -maxh 24
else
	$gmx_serial mdrun -deffnm $name -ntomp $ompN -dlb no -pin on -cpi $name.cpt -cpo $name.cpt -maxh 24 -nb gpu -gpu_id $gpu_id
fi
