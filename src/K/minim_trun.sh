...
$gmx_serial grompp -f $name.mdp -c $name.gro -p $topol_filename -o $name.tpr

if [ $gpu_id -eq -1 ]
then
    trun -m 1 -ppn $mpiN $gmx_mdrun mdrun -v -deffnm $name -ntomp $omp
else
    trun -m 1 -ppn $mpiN $gmx_mdrun mdrun -v -deffnm $name -ntomp $omp -gpu_id $gpu_id
fi
