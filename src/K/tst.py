...
for i in range(len(extra_waters)):
        ...
        my.run_it('sbatch -J gmx' + str(temp_id) + '_' + str(extra_waters[i]) + ' -p ' + partition + ' -N ' + str(mpi) + ' --ntasks-per-node=' + str(omp) + ' --wrap="python run_K_flucts.py -param_ids ' + str(temp_id) + ' -omp ' + str(omp) + ' -mpi ' + str(mpi) + ' -extra_water ' + str(extra_waters[i]) + '"')
